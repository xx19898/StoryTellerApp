...
const useEditBlock = (
	identifier: string,
	editSectionRef: React.RefObject<HTMLTextAreaElement>
) => {
	const {
		elementMap,
		setElementMap,
	...
	function onClickOutsideEditSection(e: MouseEvent) {
		if (
			...
			!editSectionRef.current.contains(e.target as Node)
		) {
			//console.log('%c CLICKED outside', 'color: red;')
			stopEditing()
			//({ elementMapInOnClickOutside: elementMap })
			removeEmptyElements()
		}
	}
