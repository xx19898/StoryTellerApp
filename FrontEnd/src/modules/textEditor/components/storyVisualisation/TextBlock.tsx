...
const EditingBlock = ({type,content,identifier}:IEditingBlock) => {
    ...
    return(
        <li className="relative" onMouseEnter={() => setToolbarVisible(true)} onMouseLeave={() => setToolbarVisible(false)}>
            {
            ...
            {
                type === 'paragraph' ? <p className="indent-4">{content}</p> : <h2 className="font-semibold text-lg py-2 my-[0.5em] text-center">{content}</h2>
            }
        </li>
    )
}
