...
const EditingInput = ({ identifier, origValue }: IEditingInput) => {
	...
	return (
		<div className='w-full h-auto flex flex-col justify-center items-center rounded-md outline outline-1 outline-white'>
			{storyUpdating ? <LoadingSpinner /> : null}
			<textarea
				spellCheck={false}
				className='indent-4 text-center p-4 h-auto w-full text-white bg-secondary focus:outline-none rounded-md resize-none'
