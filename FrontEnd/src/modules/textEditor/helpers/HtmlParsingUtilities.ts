...
export function composeHtmlElement(
	newContent: string,
...
export function addHtmlElementIdentifier(
	elType: 'h2' | 'p' | 'img',
	content: string
) {
...
export function deleteEmptyElements(
	...
	const changedArr = arr.filter((identifier) => {
		...
		const { contents } = extractTypeAndContentOfHtmlElement(
			map.get(identifier)!
		)
